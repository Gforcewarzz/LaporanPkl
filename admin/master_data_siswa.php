                        ...
                        <div class="card">
                            ...
                            <div class="card-body p-0">
                                <div class="table-responsive text-nowrap d-none d-md-block">
                                    <table class="table table-hover">
                                        ...
                                        <tbody class="table-border-bottom-0">
                                            ...
                                            // Modifikasi query agar sesuai dengan kolom yang diminta:
                                            // Asumsi:
                                            // - siswa.nama_siswa
                                            // - siswa.no_induk
                                            // - siswa.kelas
                                            // - jurusan.nama_jurusan (JOIN ke tabel jurusan)
                                            // - guru_pembimbing.nama_pembimbing (JOIN ke tabel guru_pembimbing)
                                            // - tempat_pkl.nama_tempat_pkl (JOIN ke tabel tempat_pkl)
                                            // - siswa.status
                                            $query = "SELECT 
                                                        siswa.id_siswa,
                                                        siswa.nama_siswa,
                                ...
                                <div class="d-md-none p-3">
                                    <?php
                                    // Reset result pointer untuk loop kedua (jika ada) atau jalankan query lagi
                                    mysqli_data_seek($result, 0); // Reset pointer jika $result masih valid dari atas
                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            $badgeColor = match ($row['status']) {
                                                'Tidak Aktif' => 'bg-label-warning',
